    ...
    if (authNonce->isEqualTo(result->nonce.get())) {
        ...
        if (currentDatacenter->isCdnDatacenter) {
            ...
            if (serverPublicKeys.empty()) {
                // ShuzaGram runs a single, self-hosted MTProto server (gramsrv), not
                // the real Telegram backend, so both branches point at the same key --
                // there is no separate "test" server. Regenerate this block (and the
                // fingerprint below) with Tools/genkeyfingerprint.py whenever
                // /var/lib/telesrv/server_rsa.pem is rotated on the server.
